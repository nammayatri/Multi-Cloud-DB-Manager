...
export const tableKeyOf = (table: { schema: string; table: string }): string =>
...
/**
 * Groups saved before links existed carry only the flat fkRemap map. An empty
 * parentColumns means "the parent's primary key", resolved against live
 * introspection at analyze time.
 */
...
export const linkedColumns = (table: GroupTableConfig): Set<string> =>
  new Set(effectiveLinks(table).flatMap(link => link.columns));
